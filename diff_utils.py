...
class Channel_Shuffled_Module(nn.Module):
...
class Residual_Bottleneck_Block(nn.Module):
    ...
    def __init__(self,in_channels,out_channels):
        ...
        self.branch_res1=nn.Sequential(nn.Conv2d(in_channels//2,in_channels//2,3,1,1,groups=in_channels//2),
                                    nn.BatchNorm2d(in_channels//2),
                                    ConvBnSiLu(in_channels//2,out_channels//2,1,1,0))
        self.branch_res2=nn.Sequential(ConvBnSiLu(in_channels//2,in_channels//2,1,1,0),
                                    nn.Conv2d(in_channels//2,in_channels//2,3,1,1,groups=in_channels//2),
                                    nn.BatchNorm2d(in_channels//2),
                                    ConvBnSiLu(in_channels//2,out_channels//2,1,1,0))
        self.channel_shuffle=Channel_Shuffled_Module(2)

    def forward(self,x):
        x1,x2=x.chunk(2,dim=1)
        x=torch.cat([self.branch_res1(x1),self.branch_res2(x2)],dim=1)
        x=self.channel_shuffle(x)

        return x

class Residual_Downsample_Block(nn.Module):
    '''
...
class Encoder_Block(nn.Module):
    def __init__(self,in_channels,out_channels,time_embedding_dim):
        super().__init__()
        self.conv0=nn.Sequential(*[Residual_Bottleneck_Block(in_channels,in_channels) for i in range(3)],
                                    Residual_Bottleneck_Block(in_channels,out_channels//2))

        self.time_mlp=TimeMLP(embedding_dim=time_embedding_dim,hidden_dim=out_channels,out_dim=out_channels//2)
        self.conv1=Residual_Downsample_Block(out_channels//2,out_channels)
...
class Decoder_Block(nn.Module):
    def __init__(self,in_channels,out_channels,time_embedding_dim):
        ...
        self.conv0=nn.Sequential(*[Residual_Bottleneck_Block(in_channels,in_channels) for i in range(3)],
                                    Residual_Bottleneck_Block(in_channels,in_channels//2))

        self.time_mlp=TimeMLP(embedding_dim=time_embedding_dim,hidden_dim=in_channels,out_dim=in_channels//2)
        self.conv1=Residual_Bottleneck_Block(in_channels//2,out_channels//2)
...
class Unet(nn.Module):
    '''
    simple unet design, with no attention, for a slim model size
    '''
    def __init__(self,timesteps,time_embedding_dim,in_channels=3,out_channels=2,base_dim=32,dim_mults=[2,4,8,16]):
        ...
        self.encoder_blocks=nn.ModuleList([Encoder_Block(c[0],c[1],time_embedding_dim) for c in channels])
        self.decoder_blocks=nn.ModuleList([Decoder_Block(c[1],c[0],time_embedding_dim) for c in channels[::-1]])
    
        self.mid_block=nn.Sequential(*[Residual_Bottleneck_Block(channels[-1][1],channels[-1][1]) for i in range(2)],
                                        Residual_Bottleneck_Block(channels[-1][1],channels[-1][1]//2))
